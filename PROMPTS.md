...
**What i changed next and why:** Nothing. Moved on to edit my app as there are no noticeable issues with empty state.
...
**What i changed next and why:** I carried on to edit the app with further prompts to move the gamification table around, improve the UI/UX further for readability, compatibility with my mobile interface, fixing any defects along the way. 
...
**What i changed next and why:** Nothing. I accepted the AI proposed logo for the app.
...
**What i changed next and why:** I continued to prompt to improve the layout and look and feel of the filter. 

---
## Prompt 6 - Sticky action bar fix
```
"key in exercise" and "key in food" button does not work, fix it. Change nothing else.
```
**What came back:** 5 files edited with button fixed to open up a form for the consumer to fill up.
...
```
When i use the app version, the dashboard summary shows burnt, intake and active & net vertically above each other instead of side by side in the current preview. Can you configure it such that the words are big enough, but the widgets appear side by side for iphone 15 pro max? Change nothing else.
```
**What came back:** 1 file edited with the fix deployed successfully.
...
```
**What came back:** 2 files edited with the reorder deployed successfully.
...
```
**What came back:** 1 files edited with new appearance.
...
```
**What came back:** 1 files edited with chart fixed.

**What i changed next and why:** I accepted the fix and deployed change to github.
